...
from Mobile.models import Mobile,Company
...
def addcom(request):
	if request.method=='POST':
		...
		c=connection.cursor()
		c.execute(f"insert into company( mobile) values('{cm}')")
		messages.success(request,f"{cm} is added to your database..")
...
def addmob(request):
	if request.method=='POST':
		...
		ufu = fs.url(filename)
		r10=request.POST['des']
		if Mobile.objects.filter(model=r1).exists():
			messages.error(request,"Successfully not added....")
			return redirect('addmob')
		else:
			c=connection.cursor()
			c.execute(f"""insert into mobile(mobile,model,price,quantity,
		 	  	charger,battery,headset,memory,datacable,pic,description) 
		...
		c=connection.cursor()
		c.execute("select DISTINCT(mobile) from company")
		r=c.fetchall()
...
def comlist(request):
	...
	r=c.fetchall()
	return render(request,"Mobile/comlist.html",{'data':r})


def comprofile(request):
	return render(request,"Mobile/comprofile.html")

def mobdet(request,id):
	c=connection.cursor()
	c.execute(f"select mobile,model,price,quantity,charger,battery,headset,memory,datacable,pic,description,id from mobile where id='{id}'")
	r=c.fetchone()
...
def filmob(request):
	...
	if request.method=="POST":
		...
		dt=request.POST['dt']
		if dt:
			r=f"select * from mobile where mobile like '%{dt}%'"
		if frm:
...
def delete_mobile(request,id):
	...
	return render(request,"Mobile/moblist.html")
